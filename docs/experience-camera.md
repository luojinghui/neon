...
- 心迹列表使用全局内存缓存保留已加载分页，localStorage 按身份缓存首屏 12 条、有效期 6 小时。返回页面立即展示缓存，进入页面、恢复前台或网络、以及前台每 30 秒静默更新；静默失败保留内容。发布、删除、点赞、评论同步更新缓存，期间返回的旧请求不能覆盖新操作，重复请求合并。磁盘缓存不恢复管理员权限。
- 漫游相机按“拍摄 / 选图 → 人像分析一次 → 实时调整角色 → 导出 / 分享”组织。设置分为脸部变身、立体配件、人像氛围、卡片文案，修改立即重绘，不再需要“应用到卡片”；随机变身同时组合脸部角色、配件、颜色、模板、心情和称号。
- 拍摄或选图后展示角色生成进度，资源下载按已读取字节更新，对齐五官定位、轮廓处理和海报生成阶段；结果准备完成才到 100%。欢迎卡片的“笑一下，咔嚓！”在亮暗模式下均使用深色文字与浅色底。
