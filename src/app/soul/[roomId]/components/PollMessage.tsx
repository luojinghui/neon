...
export function PollMessage({ message }: { message: ChatMessage }) {
  ...
  return (
    <>
      <section className="w-[320px] min-w-0 max-w-full overflow-hidden rounded-xl border border-border bg-surface text-left text-sm text-foreground" aria-label={message.type === 'poll-result' ? '投票最终结果' : '聊天投票'}>
