    ...
    <Modal title={title} open={open} onCancel={() => setOpen(false)} footer={null} centered destroyOnHidden width={game.kind === 'draw' ? 480 : 360}>
      <div className="space-y-4 pt-2 text-foreground">
        ...
        {game.kind === 'draw' && <>
          ...
          {!finished && game.guesses.length > 0 && <div className="space-y-1 text-xs text-foreground-muted" aria-label="最近的猜测">{game.guesses.slice(-3).map((item, index) => <p key={`${item.timestamp}-${index}`} className="break-words">{item.name}：{item.text}</p>)}</div>}
