    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4 md:gap-6">
      {items.map((_, idx) => (
        <div
          ...
          className="rounded-lg border border-border p-4 bg-surface backdrop-blur-sm shadow-sm"
          aria-hidden="true"
        >
          ...
          <div className="mt-4 h-7 w-16 rounded-lg bg-background-tertiary animate-pulse" />
        </div>
      ))}
    </div>
  );
