...
export function PortraitControls({ value: draft, title, busy, faceCount, segmented, available, hint, onChange, onSurprise }: Props) {
  ...
  return <div className="portrait-controls">
    ...
    <details open><summary>脸部变身 <span>贴合五官 · 实时预览</span></summary>
      <div className="portrait-sticker-grid">{FACE_EFFECTS.map(effect => <button key={effect.id} type="button" aria-pressed={draft.faceEffect === effect.id} disabled={busy || (!faceCount && effect.id !== 'none')} onClick={() => patch({ faceEffect: effect.id, ...(effect.id === 'cat' || effect.id === 'fox' ? { sticker: 'cat', stickerColor: effect.id === 'cat' ? '#ffdc7b' : '#f69d4c' } : effect.id === 'panda' ? { sticker: 'bear', stickerColor: '#34313e' } : effect.id === 'avatar' ? { sticker: 'none' } : {}) })}><span aria-hidden="true">{effect.icon}</span>{effect.name}</button>)}</div>
      {slider('变身浓度', 'faceEffectStrength', 0, 100, 1, !faceCount || draft.faceEffect === 'none')}
    </details>
